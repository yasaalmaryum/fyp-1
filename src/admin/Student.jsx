...
const StudentR = () => {
  ...
  const fetchStudents = async () => {
    ...
    try {
      const response = await axios.get(URLS.ListStudent, {
        headers: headers,
      });
  ...
  const handleSubmit = (e) => {
    ...
    const apiRequestBody = {
      ...
      section: "A", // Assuming section is a constant value, change as needed
    ...
    if (selectedUser) {
      ...
      // Update student
      axios
        .put(URLS.deleteStudent.replace(":id", selectedUser), apiRequestBody, {
          headers: headers,
        })
        .then((res) => {
      ...
      // Create new student
      axios
        .post(URLS.createStudent, apiRequestBody, { headers: headers })
        .then((res) => {
  ...
  const handleUpdate = (index) => {
    ...
    setRegistrationNumber(userToUpdate.reg_no);
  ...
  const confirmDelete = async () => {
    if (selectedUser) {
      ...
      try {
        ...
        await axios.delete(URLS.deleteStudent.replace(":id", selectedUser), {
          headers,
        });
      ...
      {users.length > 0 && (
        <div className="mt-8">
          ...
          <table className="w-full border">
            ...
            <tbody>
              {users
                .filter((user) => user.user.is_student) // Filter to only include students
                .map((user, index) => (
                  <tr
                    key={index}
                  ...
                  >
                    ...
                    <td className="border px-4 py-2">{user.reg_no}</td>
